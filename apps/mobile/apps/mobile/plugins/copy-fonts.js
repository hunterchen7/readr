...
module.exports = function copyAssetsPlugin(config) {
  return withDangerousMod(config, [
    ...
    (cfg) => {
      ...
      const androidAssets = path.join(cfg.modRequest.platformProjectRoot, "app", "src", "main", "assets");
...
function copyDir(src, dest, exts) {
  ...
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) copyDir(srcPath, destPath, exts);
    else if (exts.some((e) => entry.name.endsWith(e))) fs.copyFileSync(srcPath, destPath);
  }
}
