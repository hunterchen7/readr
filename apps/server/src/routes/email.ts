...
emailPublicRouter.post("/email/recover/start", async (c) => {
  ...
  // With the partial unique index on (email) WHERE email_verified_at
  // IS NOT NULL, at most one verified user can exist per email.
  const [user] = await db
...
emailPublicRouter.post("/email/recover/finish", async (c) => {
  ...
  // Look up the bearer token for this user (row.userId is the UUID).
  const [user] = await db
    .select({ token: schema.users.token })
    .from(schema.users)
  ...
  await sendDeviceTokenRecovery({ to: email, token: user.token });
...
emailPublicRouter.post("/email/login", async (c) => {
  ...
  // With the partial unique index, at most one verified user per email.
  ...
  let user = existingUser as { id: string } | undefined;

  if (!user) {
    // Auto-create user with a generated bearer token. The UUID PK is
    // generated by the database.
    const token = generateToken();
    const [newUser] = await db
      .insert(schema.users)
      .values({ token, email, emailVerifiedAt: new Date() })
...
emailRouter.post("/email/verify", async (c) => {
  ...
  // Check that no other user has already verified this email.
  const [conflict] = await db
    .select({ id: schema.users.id })
    .from(schema.users)
    .where(
      and(
        eq(schema.users.email, row.email),
        isNotNull(schema.users.emailVerifiedAt),
      ),
    )
    .limit(1);
  ...
  if (conflict && conflict.id !== userId) {
    throw badRequest("This email is already verified by another account");
  }
